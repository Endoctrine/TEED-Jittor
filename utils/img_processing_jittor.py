...
def save_image_batch_to_disk(tensor, output_dir, file_names, img_shape=None, arg=None, is_inchannel=False):
    ...
    if not arg.is_testing:
        ...
        for tensor_image, file_name in zip(tensor, file_names):
            image_vis = jittor.sigmoid(tensor_image).numpy()  # [..., 0]
            image_vis = (255.0 * (1.0 - image_vis)).astype(np.uint8).squeeze()
            output_file_name = str(os.path.join(output_dir, file_name))
        ...
        if is_inchannel:
            ...
            edge_maps2 = []
            for i in tensor2:
                tmp = jittor.sigmoid(i).detach().numpy()
        ...
        image_shape = [x.numpy() for x in img_shape]
        # (H, W) -> (W, H)
